        ...
        function generateCSV(logs, period) {
            if (logs.length === 0) {
            ...
            let csv = `Study Report for ${period}\n\n`;
            csv += `Total Study Time,${formatTime(totalStudySeconds)}\n`;
            csv += `Total Break Time,${formatTime(totalBreakSeconds)}\n\n`;
            csv += `SESSION DETAILS\n`;
            csv += `Session Start,Session End,Total Study Duration,Total Break Duration,Number of Breaks\n`;

            const sortedLogs = [...logs].sort((a, b) => a.sessionStartTime.toMillis() - b.sessionStartTime.toMillis());

            sortedLogs.forEach(log => {
                const sessionStart = log.sessionStartTime.toDate().toLocaleString();
                const sessionEnd = log.sessionEndTime.toDate().toLocaleString();
                const studyDuration = formatTime(log.totalStudyDurationSeconds);
                const breakDuration = formatTime(log.totalBreakDurationSeconds);
                const breakCount = log.breaks.length;
                csv += `"${sessionStart}","${sessionEnd}",${studyDuration},${breakDuration},${breakCount}\n`;
            });

            csv += `\n\nBREAK DETAILS\n`;
            csv += `Session Start,Break Start,Break End,Break Duration,Reason\n`;
            sortedLogs.forEach(log => {
                if (log.breaks && log.breaks.length > 0) {
                    log.breaks.forEach(b => {
                        const sessionStart = log.sessionStartTime.toDate().toLocaleString();
                        const breakStart = b.breakStartTime.toDate().toLocaleString();
                        const breakEnd = b.breakEndTime.toDate().toLocaleString();
                        const breakDuration = formatTime((b.breakEndTime.toDate() - b.breakStartTime.toDate()) / 1000);
                        const reason = `"${b.reason.replace(/"/g, '""')}"`;
                        csv += `"${sessionStart}","${breakStart}","${breakEnd}",${breakDuration},${reason}\n`;
                    });
                }
            });
        ...
        downloadDailyBtn.addEventListener('click', () => {
            ...
            const csv = generateCSV(filteredLogs, `the date ${date}`);
            if (csv) downloadCSV(csv, `study_report_daily_${date}.csv`);
        });
        ...
        downloadWeeklyBtn.addEventListener('click', () => {
            ...
            const csv = generateCSV(filteredLogs, `the week starting ${start.toLocaleDateString()}`);
            if (csv) downloadCSV(csv, `study_report_weekly_${week}.csv`);
        });
        ...
        downloadMonthlyBtn.addEventListener('click', () => {
            ...
            const csv = generateCSV(filteredLogs, `the month of ${start.toLocaleString('default', { month: 'long', year: 'numeric' })}`);
            if (csv) downloadCSV(csv, `study_report_monthly_${month}.csv`);
        });
